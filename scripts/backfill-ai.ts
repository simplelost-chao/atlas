...
dotenv.config({ path: path.resolve("/Users/chao/Documents/Projects/atlas/.env") });
dotenv.config({ path: path.resolve("/Users/chao/Documents/Projects/atlas/.env.local"), override: true });
...
async function main() {
  const companies = await db.company.findMany({
    where: { moat: null, marketPosition: { in: ["LEADER","CHALLENGER"] }, chainNode: { chain: { project: { industry: "AI" }, status: "COMPLETED" } } },
    include: { chainNode: true },
    orderBy: { createdAt: "asc" },
  });
  console.log(`\n🔬 AI 深度分析: ${companies.length} 家\n`);
  ...
  for (const c of companies) {
    ...
    const t1 = Date.now();
    try {
      const { data, cost: cc } = await analyze(c.name, "AI", c.chainNode.name);
      ...
      await db.$executeRawUnsafe(`INSERT INTO "AnalysisLog" ("companyId","companyName",industry,"nodeName",status,"durationSec","costUsd") VALUES ($1,$2,$3,$4,$5,$6,$7)`,
        c.id, c.name, "AI", c.chainNode.name, "success", dur, cc);
      ...
      await db.$executeRawUnsafe(`INSERT INTO "AnalysisLog" ("companyId","companyName",industry,"nodeName",status,"durationSec") VALUES ($1,$2,$3,$4,$5,$6)`,
        c.id, c.name, "AI", c.chainNode.name, "failed", dur).catch(()=>{});
